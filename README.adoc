...
* *Defined Units*: All metrics include proper UCUM units (e.g., `s`, `ms`, `By`, dimensionless, etc.) for consistent interpretation across observability platforms
...
5. Most importantly – keep in place existing testing infrastructure as they will continue verifying metrics are still being exported.
...
|Gauge
|`1`|Indicates whether this member is the current coordinator (1=coordinator, 0=not coordinator). Cluster coordination indicator. Protocol name is lowercase (e.g., `jgroups.ping.is_coord`, `jgroups.dns_ping.is_coord`)
...
|`jgroups.<protocol>.is_coord`
|Gauge
|`1`|Indicates whether this member is the current coordinator (1=coordinator, 0=not coordinator). Cluster coordination indicator. Protocol name is lowercase (e.g., `jgroups.file_ping.is_coord`, `jgroups.jdbc_ping.is_coord`)
|===
...
|Gauge
|`1`|Number of cached ViewIds from different subgroups. Indicates potential network partitions detected

|`jgroups.merge3.merge_events`
|Counter
|`1`|Number of times a MERGE event was sent up the stack. Tracks merge operations initiated

|`jgroups.merge3.view_consistency_checker.running`
|Gauge
|`1`|Indicates whether the view consistency checker task is running (1=running, 0=stopped). Task activity indicator

|`jgroups.merge3.info_sender.running`
|Gauge
|`1`|Indicates whether the info sender task is running (1=running, 0=stopped). Task activity indicator
...
|Gauge (Configuration)
|`1`|Maximum number of merge participants to be involved in a merge (0=unlimited)
|===
...
|Gauge
|`1`|Number of currently suspected members

|`jgroups.fd_sock.suspect_events`
|Counter
|`1`|Number of suspect events generated

|`jgroups.fd_sock.monitor.running`
|Gauge
|`1`|Indicates whether the node crash detection monitor is running (1=running, 0=stopped)
...
|Gauge (Configuration)
|`1`|Maximum number of elements in the cache until deleted elements are removed
...
|Gauge (Configuration)
|`1`|Number of attempts coordinator is solicited for socket cache until we give up

|`jgroups.fd_sock.port.start`
|Gauge (Configuration)
|`1`|Start port for server socket (0 picks random port)

|`jgroups.fd_sock.port.client_bind`
|Gauge (Configuration)
|`1`|Start port for client socket (0 picks random port)

|`jgroups.fd_sock.port.range`
|Gauge (Configuration)
|`1`|Number of ports to probe for start_port and client_bind_port

|`jgroups.fd_sock.keep_alive`
|Gauge (Configuration)
|`1`|Whether to use KEEP_ALIVE on the ping socket (1=enabled, 0=disabled)
|===
...
|Gauge (Configuration)
|`1`|Offset from the transport's bind port

|`jgroups.fd_sock2.port.range`
|Gauge (Configuration)
|`1`|Number of ports to probe for finding a free port
...
|Gauge (Configuration)
|`1`|Lowest port the FD_SOCK2 server can listen on

|`jgroups.fd_sock2.port.max`
|Gauge (Configuration)
|`1`|Highest port the FD_SOCK2 server can listen on
...
|Counter
|`1`|Number of suspect events sent

|`jgroups.fd_all3.has_suspected_members`
|Gauge
|`1`|Indicates whether there are currently any suspected members (1=yes, 0=no)

|`jgroups.fd_all3.timeout_checker.running`
|Gauge
|`1`|Indicates whether the timeout checker task is running (1=running, 0=stopped)

|`jgroups.fd_all3.heartbeat_sender.running`
|Gauge
|`1`|Indicates whether the heartbeat sender task is running (1=running, 0=stopped)
...
|Gauge (Configuration)
|`1`|Number of bits for each member (timeout / interval)
|===
...
|Gauge (Configuration)
|`1`|Whether to use TimeService for timestamps (1=enabled, 0=disabled)
|===
...
|Gauge
|`1`|Indicates whether the verification task is currently running (1=running, 0=stopped). Task activity indicator
|===
...
|Gauge
|`1`|Number of STABLE votes received for current digest.

|`jgroups.pbcast.stable.suspended`
|Gauge
|`1`|Indicates whether garbage collection is suspended (1=suspended, 0=active).

|`jgroups.pbcast.stable.stable_task.running`
|Gauge
|`1`|Indicates whether the stable task is running (1=running, 0=stopped).
...
|Gauge
|`1`|Total number of views installed in this member. View installation tracking indicator

|`jgroups.pbcast.gms.is_coord`
|Gauge
|`1`|Indicates whether this member is the current coordinator (1=coordinator, 0=not coordinator). Cluster coordination indicator

|`jgroups.pbcast.gms.is_leaving`
|Gauge
|`1`|Indicates whether this member is in the process of leaving (1=leaving, 0=not leaving). Member departure indicator

|`jgroups.pbcast.gms.merge.in_progress`
|Gauge
|`1`|Indicates whether a merge is currently in progress (1=merging, 0=not merging). *Critical*: Merge activity indicator

|`jgroups.pbcast.gms.merge.task.running`
|Gauge
|`1`|Indicates whether the merge task is currently running (1=running, 0=stopped). Merge task activity indicator

|`jgroups.pbcast.gms.merge.killer.running`
|Gauge
|`1`|Indicates whether the merge killer task is currently running (1=running, 0=stopped). Merge timeout task activity indicator

|`jgroups.pbcast.gms.view_handler.queue`
|Gauge
|`1`|Number of queued view change requests (JOIN/LEAVE/SUSPECT) waiting to be processed. *Critical*: High values indicate processing backlog

|`jgroups.pbcast.gms.view_handler.suspended`
|Gauge
|`1`|Indicates whether the view handler is suspended (1=suspended, 0=active). Handler activity indicator
...
|Gauge (Configuration)
|`1`|Maximum number of join attempts before giving up and becoming singleton (0 means never give up)
|===
...
|`jgroups.ufc.blocked`
|Gauge
|`1`|Number of times flow control blocked a sender waiting for credits. *Critical*: High values indicate backpressure
...
|Gauge (Configuration)
|`1`|Threshold (as percentage of max_credits) at which a receiver sends more credits
